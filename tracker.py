...
class tracker():
    ...
    def __init__(self, Mywindow_width, Mywindow_height, Mymargin, My_ym=1, My_xm=1, Mysmooth_factor=15):
        ...
        #the window pixel height of the center valules, used to count pixels inside center windows to determine curve values
    ...
    def find_window_centroids(self, warped):
        ...
        window_centroids= [] #stops the (left,right) window centroid positions per level
        window = np.ones(window_width) #create our window template that we will use for convolutions

        #First find the two starting postions for the left and right lane by using np.sum to get the vertical image slice 
